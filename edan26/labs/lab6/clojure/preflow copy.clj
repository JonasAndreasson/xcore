...
(defn push [edge-index u nodes edges excess-nodes change s t]
	...
	(if (has-excess u nodes) (do (check-insert excess-nodes u s t)))
		...
		(do
			(println "--------- push -------------------")
			(println "u is u = " (u-is-edge-u @(edges i) u)) 
			(println "i = " i)
			(println "u = " u)
			(println "uh = " uh)
			(println "e = " e)
...
(if (and (not (empty? adj))(has-excess u nodes)) ;if adjency is not empty and we have excess
(let [updated-change (try-push u nodes (first adj) edges excess-nodes s t)] ;try push the first edge
(rec-edge u nodes (rest adj) edges excess-nodes updated-change s t);and recurs the rest
))
...
(defn preflow []
	...
	(let [threads (repeatedly num-threads #(Thread. make-preflow))]
		(run! #(.start %) threads)
		(run! #(.join %) threads))
